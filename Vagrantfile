...
#  config.ssh.insert_key = false
  ...
  config.vm.provision "shell", name: "yum update then reboot", inline: "sudo yum update -y"
  # reload requires: vagrant plugin install vagrant-reload
  ...
  #config.vm.provision "shell", name: "setup mariadb", path: "installmysql.sh" 	
  ...
  config.vm.provision "shell", run: "always", name: "Show mysql startup status", inline: "sudo service mysql status"
	...
	#config.vm.synced_folder "/Users/joey.mart/Downloads/glide-db-dumps/", "/glide-db-dumps", :mount_options => ["dmode=777", "fmode=666"]
	#config.vm.synced_folder "/Users/joey.mart/repositories/server/", "/mariadb-source/server", :mount_options => ["dmode=777", "fmode=666"]
